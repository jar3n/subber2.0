...
def main():
    ...
    api_key_file = Path(f"{os.path.dirname(os.path.abspath(getsourcefile(lambda:0)))}/api_key")
    if not api_key_file.is_file():
        print("ERROR: cannot find the api_key file with you youtube API key in it. Make sure the file is in the same directory as the subber.py file.")
        return 
    else:
        ...
        try:
            ...
            print("ERROR: api key file not properly formatted. Make sure there is a line with \'[key]\' followed by a line with \'api_key=YOUR_API_KEY\'")
            return
        api_key = cp['key']['api key']

    parser = argparse.ArgumentParser(description="A tool for tracking youtube subscriptions locally!")
    ...
    parser.add_argument('-f',
                        '--set_update_freq', 
                        nargs=2, 
                        help="set the number of days to wait before checking if a channel has uploaded.", 
    ...
    parser.add_argument("-w",
                        ...
                        type=str, 
                        nargs='*', 
                        help="set a channel's latest video to not appear in list because you watched it unless they upload a new video", 
                        metavar='<channel handle>')
